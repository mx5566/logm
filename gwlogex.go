...
func ParseLevel(s string) Level {
	...
	ErrorfE("ParseLevel: unknown level: %s", s)
	return DebugLevel
}
...
func Init(appname string, fileConfig map[string]string, level string) {
	SetSourceEx(appname)
	SetOutputEx(fileConfig)
	SetLevelEx(ParseLevel(level))
}
